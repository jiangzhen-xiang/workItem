<template>
  <div>
    <Form ref="formValidate"
      ...
      <FormItem label="所属集群"
                prop="cluster">
        <Select v-model="formValidate.cluster"
                placeholder="请选择">
          <Option v-for="(item, index) in typeArr"
                  :key="index"
                  :value="item.id.toString()"
                  :label="item.showName+'-'+item.name">{{ item.showName }}-{{ item.name }}</Option>
        </Select>
      </FormItem>
      <FormItem label="用户名称"
                prop="user">
      ...
      <FormItem label="分区数"
                prop="partitionCount">
        <Input v-model="formValidate.partitionCount"
               placeholder="请输入分区数"
               minlength="5"
               maxlength="20"
               show-word-limit></Input>
      ...
      <FormItem label="备份数"
                prop="replicateCount">
        <Input v-model="formValidate.replicateCount"
               placeholder="请输入备份数"
               minlength="5"
               maxlength="20"
               show-word-limit
