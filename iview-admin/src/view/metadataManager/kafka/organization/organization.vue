...
<template>
  <div>
      ...
      <i-col span="24">
        <Card>
          <p slot="title" style="height:34px;">
            ...
            <FormItem  prop="name">
              <Input v-model="formSearch.name" placeholder="应用用户名称" size="small"></Input>
            </FormItem>
            <FormItem>
    ...
    <Modal v-model="modalColony" title="增加应用用户" footer-hide>
      <div class="formWrapper">
          <colonyForm  @handleSuccess="handleColonyAdd" :index="index" @handleCancel="handleCancel"/>
      </div>
    </Modal>
    <Modal v-model="modalEditorColony" title="编辑应用用户" footer-hide>
      <div class="formWrapper">
...
import { ajaxGetUserMeta, ajaxDeleteUserMeta, ajaxAddUserMeta, ajaxEditorUserMeta } from '@/api/datametaCluster.js'
...
export default {
  ...
  methods: {
    ...
    handleDeleteEnvironment(row) {
      this.loading = true
      ajaxDeleteUserMeta(row).then(res => {
        this.loading = false
    ...
    getColonyArr() {
      ...
      this.loading = true
      ajaxGetUserMeta(postData).then(res => {
        this.loading = false
        if (res.data.status == '1') {
