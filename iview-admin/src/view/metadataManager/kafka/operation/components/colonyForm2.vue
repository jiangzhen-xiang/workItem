<template>
  <div>
    ...
    >
      ...
      <FormItem label="保存时长(秒)" prop="retentionMs">
        <Input v-model="formValidate.retentionMs" placeholder="请输入"></Input>
      </FormItem>
      <!-- <FormItem label="topic备份数" prop="replicateCount">
        <Input
          v-model="formValidate.replicateCount"
          placeholder="请输入topic备份数"
          minlength="5"
          maxlength="20"
          show-word-limit
        ></Input>
...
<script>
import { ajaxAddScriptMeta, ajaxGetClusterMeta, ajaxGetTopic } from '@/api/datametaCluster.js'
export default {
  name: 'colonyForm',
  data() {
  ...
  methods: {
    ...
    ok() {
      ...
      this.$refs.formValidate.validate(validate => {
        if (validate) {
          ajaxAddScriptMeta(postData).then(res => {
            if (res.data.status == '1') {
              this.$Message.success('添加成功')
              this.handleReset('formValidate')
              this.$emit('handleSuccess', 'modalColony2')
            } else {
    ...
    cancel() {
      this.handleReset('formValidate')
      this.$emit('handleCancel', 'modalColony2')
    },
    handleAddAnnotions() {
