...
<script>
import { ajaxEditorScriptMeta, ajaxGetClusterMeta, ajaxGetTopic } from '@/api/datametaCluster.js'
export default {
  name: 'colonyForm',
  props: ['row'],
  data () {
  ...
  methods: {
        ...
        if (validate) {
          ajaxEditorScriptMeta(postData).then(res => {
            if (res.data.status == '1') {
              ...
              this.$emit('handleSuccess', 'modalEditorColony2')
            } else {
    ...
    cancel () {
      ...
      this.$emit('handleCancel', 'modalEditorColony2')
    },
    handleAddAnnotions () {
