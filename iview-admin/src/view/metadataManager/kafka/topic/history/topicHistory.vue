...
<template>
  <div>
      ...
      <i-col span="24">
        <Card>
          <p slot="title" style="height:34px;">
            ...
            <FormItem  prop="name">
              <Input v-model="formSearch.name" placeholder="Topic名称" size="small"></Input>
            </FormItem>
            <FormItem>
          ...
          <div class="btnWrapper" slot="extra" style="left:16px;">
            <ButtonGroup>
              ...
              <Button @click="handleAdd2" type="primary" size="small">
                ...
                <template slot-scope="{ row,index }" slot="action">
                   <Dropdown @on-click="handleDropdownMenu(row,$event)" transfer>
                      ...
                      <DropdownMenu slot="list" >
                          <DropdownItem name="Editor" :disabled="row.status == 2"><div style="color:#2db7f5"><Icon type="ios-create-outline" />  编辑</div></DropdownItem>
                          <!-- <DropdownItem name="Delete"><Icon type="ios-trash-outline" />  删除</DropdownItem> -->
                          <DropdownItem name="Run" :disabled="row.status == 2"><div style="color:#19be6b"><Icon type="ios-navigate-outline" />  在集群中生效</div></DropdownItem>
                      </DropdownMenu>
                  </Dropdown>
                </template>
            </Table>
    ...
    <Modal v-model="modalColony" title="创建Topic" footer-hide>
      <div class="formWrapper">
          <colonyForm  @handleSuccess="handleColonyAdd" :index="index" @handleCancel="handleCancel"/>
      </div>
    </Modal>
    <Modal v-model="modalEditorColony" title="创建" footer-hide>
      <div class="formWrapper">
...
import { ajaxGetHistoryMeta, ajaxRunHistoryMeta } from '@/api/datametaCluster.js'
...
export default {
  ...
  methods: {
    ...
    handleFireDropdownRun(row) {
      this.$Modal.confirm({
        title: '消息提示',
        closable: true,
        content: '运行该数据后将不可修改，是否运行?',
        loading: true,
        onOk: () => {
          this.handleRunEnvironment(row)
          this.$Modal.remove()
          this.modal1 = false
        }
      })
    ...
    handleRunEnvironment(row) {
      this.loading = true
      ajaxRunHistoryMeta(row).then(res => {
        this.loading = false
        if (res.data.status == '1') {
          this.$Message.success('数据在集群运行成功')
          this.getColonyArr()
        } else {
          this.$Message.error('数据在集群运行失败')
        }
      }).catch(err => {
        this.loading = false
    ...
    getColonyArr() {
      ...
      this.formSearch.name && (postData.topic = this.formSearch.name)
      this.loading = true
      ajaxGetHistoryMeta(postData).then(res => {
        this.loading = false
        if (res.data.status == '1') {
