...
<template>
  <div>
        ...
        <Card>
          <p slot="title" style="height:34px;">
            <Form ref="formSearch" :model="formSearch" :rules="ruleValidate" inline >
              <FormItem  prop="name">
                <Input v-model="formSearch.name" placeholder="名称" size="small"></Input>
              </FormItem>
              <FormItem  prop="cluster">
                <Input v-model="formSearch.cluster" placeholder="集群简称" size="small"></Input>
              </FormItem>
          <FormItem>
            ...
            <Table :loading="loading" :columns="tableTitle"  tooltip-theme="dark" :data="tableData" border :height="tableHeight">
              <template slot-scope="{ row,index }" slot="name">
                ...
                <template slot-scope="{ row }" slot="environment">
                    <Select v-model="row.defaultEnv" prop="defaultEnv" transfer placeholder="环境"  size="small" >
                ...
                <template slot-scope="{ row }" slot="retentionMs">
                   <span><span class="" style="color:#19be6b">{{ getTime(row)}}</span>(秒)</span>
                </template>
                <template slot-scope="{ row,index }" slot="action">
                   <Dropdown @on-click="handleDropdownMenu(row,$event)" transfer>
                      ...
                      <DropdownMenu slot="list" >
                        <DropdownItem name="Analysis" ><div style="color:#2db7f5"><Icon type="ios-analytics-outline" />  统计分析</div></DropdownItem>
                          <DropdownItem name="Group" ><div style="color:#19be6b"><Icon type="logo-buffer" />  查看Group</div></DropdownItem>
                          <DropdownItem name="Warrant" ><div style=""><Icon type="ios-git-branch" />  查看授权</div></DropdownItem>
    ...
    <Modal v-model="modalColony" title="增加主题" footer-hide>
      <div class="formWrapper">
          <colonyForm  @handleSuccess="handleColonyAdd" :index="index" @handleCancel="handleCancel"/>
      </div>
    </Modal>
    <Modal v-model="modalEditorColony" title="编辑主题" footer-hide>
      <div class="formWrapper">
    ...
    <Modal v-model="modalLook" :title="lookTitle" footer-hide width="80%">
      <div class="formWrapper">
          <commonTable  @firePageChange="handlePageChange" :tableTitle="lookTableColumn" :tableData="lookTableData" :current="lookCurrent" :postData="paramPostData" :total="lookPageTotal" :page-size="lookPageSize"/>
      </div>
    </Modal>
    <Modal v-model="modalChart" :title="lookTitle" footer-hide width="80%">
      <div class="formWrapper">
        <Card v-for="(item,index) in chartList">
        <p slot="title">
            <Icon type="ios-analytics" />
            {{item.group}}
        </p>
        <p ><commonChart :ref="'commonChart_'+item.group"  :option="item.optionData" /></p>
...
import {
  ajaxGetTopic,
  ajaxGetTopicByGroup,
  ajaxGetWarrantByTopic,
  ajaxGetGroupByTopic,
  ajaxGetAnalysisByTopic,
  ajaxGetTopicInfo, ajaxGetWarrantInfo
...
export default {
  ...
  methods: {
    ...
    handleSetChartList(res, type) {
      ...
      Object.keys(res.data.data).map(key => {
        ...
        let option = {
          ...
          legend: {
            data: ['写入数据总量', '已消费数据总量', '积压数据总量', '消费开始量']
          },
          xAxis: {
            data: [],
            boundaryGap: false,
        ...
        res.data.data[key].map(item => {
          // xArr.unshift(that.$timeFormat(new Date(item.timestamp)))
          xArr.unshift(item.timestamp)
          s1Arr.unshift(item.totalOffset)
          s2Arr.unshift(item.commitOffset)
          s3Arr.unshift(item.lagOffset)
          s4Arr.unshift(item.startOffset)
        })
        option.xAxis.data = xArr
        option.series[0].data = s1Arr
        option.series[1].data = s2Arr
        option.series[2].data = s3Arr
        option.series[3].data = s4Arr
        obj.optionData = option
        this.chartList.push(obj)
      })

      // this.lookTableData = res.data.data.map(item => {
      //   xArr.unshift(that.$timeFormat(new Date(item.timestamp)))
      //   s1Arr.unshift(item.totalOffset)
      //   s2Arr.unshift(item.commitOffset)
      //   s3Arr.unshift(item.lagOffset)
    ...
    getChartData(postData, timer, row) {
      let that = this
      ajaxGetAnalysisByTopic(postData).then(res => {
        if (res.status == 200) {
          this.handleSetChartList(res, true)
          that.$nextTick(() => {
            this.chartList.map(item => {
              that.$refs['commonChart_' + item.group][0].handleFireChart()
            })
            this.handleLoop(timer, row, res)
          })
        } else {
    ...
    handleLoop(timer, row, res) {
      ...
      let postData = {
        ...
        name: row.name
        // requestType: 1,
        // end: res.data.data[0] && res.data.data[0].timestamp
      }
      setTimeout(() => {
        if (that.modalChart) {
          this.getChartData(postData, timer, row)
        }
      }, 100)
    },
    getTime(row) {
      let text = (row.retentionMs || 0) / 1000
      return text
    },
    ...
    handleFireDropdownGroup(row) {
      ...
      ajaxGetGroupByTopic(postData).then(res => {
        console.log(res)
        this.loading = false
        ...
        if (res.data.status == 1) {
          ...
          console.log(this.lookTableData,1)
    ...
    handleFireDropdownWarrant(row) {
      ...
      ajaxGetWarrantByTopic(postData).then(res => {
        this.loading = false
        if (res.data.status == '1') {
          this.lookTableData = res.data.data.list
          this.lookPageTotal = res.data.data.total
    ...
    handleFireDropdownAnalysis(row) {
      let that = this
      this.lookTitle = '统计分析(' + row.name + ')'
      let postData = {
        pageSize: 10,
        pageNum: 1,
        cluster: row.cluster,
      ...
      ajaxGetAnalysisByTopic(postData).then(res => {
        console.log(res,'统计分析-------------------')
        if (res.data.status == 1) {
          this.handleSetChartList(res)
          console.log(111111)
          that.$nextTick(() => {
            this.chartList.map(item => {
              that.$refs['commonChart_' + item.group][0].handleFireChart()
            })
            this.handleLoop(1000, row, res)
          })
        } else {
          this.$Message.error('查看授权信息失败')
        }
