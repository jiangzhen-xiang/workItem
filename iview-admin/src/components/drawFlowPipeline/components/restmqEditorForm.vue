<template>
  <div>
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
      ...
      <FormItem label="hostname" prop="server">
        <Input v-model="formValidate.server" placeholder="请输入"></Input>
      </FormItem>
...
export default {
  ...
  methods: {
    ok() {
      ...
      this.$emit('handleSuccessEditor', postData)
      // this.handleReset("formValidate");
    },
    cancel() {
      this.handleReset('formValidate')
      this.$emit('handleCancel', 'modal2')
    },
    handleAddAnnotions() {
    ...
    handleReset(name) {
      ...
      for (let k in this.formValidateEditor) {
        this.formValidate[k] = this.formValidateEditor[k]
      }

      if (this.formValidateEditor.annotions.length == 0) {
        this.formValidateEditor.annotions = []
      }
    ...
    getFormData() {
      ...
      postData.type = 'restmq'
      postData.index = this.curIndex
